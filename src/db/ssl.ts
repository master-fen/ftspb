// Расширение обязательно: этот модуль лежит на пути импорта у скриптов,
// которые запускает `node` (scripts/parse-archive.ts → archive-migration-rules
// → сюда). Vite и bun разрешают обе формы, а node с разбором типов — только
// явную, и без расширения весь разбор архива падает ERR_MODULE_NOT_FOUND.
import { TIMEWEB_CA } from "./ca.ts";
