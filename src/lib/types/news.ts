...
> & {
  /**
   * Обложку пришлось бы заметно увеличивать, чтобы заполнить область карточки
   * (`isSmallCover`, src/lib/card-cover.ts) — она показывается целиком, по
   * центру, на поверхности карточки. Решение принимает сервер по
   * `news_photo.width/height`, до отрисовки.
   *
   * Только `true` и только когда обложка действительно маленькая: ключа со
   * значением `false` в объекте быть не должно. Данные лоадера уезжают в
   * SSR-разметку, и постоянный ключ изменил бы разметку каждой страницы с
   * карточками — в том числе там, где вид не меняется.
   */
  coverSmall?: true;
};
