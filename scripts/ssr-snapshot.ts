 ...
 * исходнику в dev-атрибуте, дата «сегодня» в данных лоадера событийных
 * страниц). Списки preload не выбрасываются, а выносятся в
...
export function normalizeHtml(
  ...
  const split = html
    ...
    // 2а) дата «сегодня» в данных лоадера событийных страниц (todayInMoscow):
    // та же сборка в другой день отличалась бы только ею. Прочие даты не трогаются.
