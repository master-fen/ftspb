...
- Публичный адрес новости — `slug`, уникальный. Существующие ссылки
  `/news/$newsId` не ломаются — с 6 исключениями из 41 записи архива
  (см. «Известные расхождения slug» ниже).
...
`migrate-archive.ts` строит `slug` через `slugify(title)`; при коллизии
заголовков — суффикс полной ISO-даты. Для 35 из 41 записи это совпало с
идентификатором, который до миграции использовал `mock.ts`/`news-archive.ts`
(и, соответственно, URL `/news/$newsId` на сайте). Для 6 записей — нет,
адрес осознанно изменился (решение Антона, 04.08.2026):

- 3 записи — коллизия заголовка «Чествование Мастеров спорта России»
  (три разные даты). В `mock.ts` дедуплицировано вручную суффиксом
  `-2`/`-2-2`, `migrate-archive.ts` разводит суффиксом ISO-даты
  (`…-2026-03-18`, `…-2026-02-19`, `…-2026-02-14`).
- 3 записи — `mock.ts`/старый сайт обрезали slug до **60 символов** (не 61)
  и убирали хвостовой дефис после обрезки; `slugify()` в `migrate-archive.ts`
  такого лимита не применяет, slug в БД — полный:
  `pervenstvo-sankt-peterburga-sredi-yuniorov-i-yuniorok-do-19` →
  `…-do-19-let`; `torzhestvennoe-vruchenie-nagrudnogo-znaka-za-zaslugi-v-razvi`
  → `…-v-razvitii-fizicheskoy-kultury-i-sporta-sankt-peterburga`;
  `otchetno-vybornoe-sobranie-chlenov-federatsii-tennisa-sankt` →
  `…-tennisa-sankt-peterburga`.

Для уже смигрированных 41 записей slug в БД не меняем задним числом — цена
внешних ссылок на эти 6 новостей практически нулевая. На будущее (этап 8,
полный архив 2006–2025) `slug.ts` умеет обрезать по той же границе в 60
символов, чтобы новые slug совпадали со старым сайтом — см. `slug.ts`.
