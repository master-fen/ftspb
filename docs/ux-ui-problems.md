...
- **Прозрачность цвета вместо токена приглушения вне ролей подписи.** Правило запрещает
  ...
  `_site.news.$newsId.tsx:163`, `:174`, `text-brand-navy/70`, `/60`). На тёмном фоне — открытый вопрос
  «Поверхностей» (текст на тёмном фоне): подвал (`SiteFooter.tsx:21`, `:33`, `text-white/90`; реквизиты `:31`, `:44`,
...
- **Радиусы блоков не унифицированы.** Карточки — `rounded-xl` (`ui-card`, #75), обёртка секции
  ...
  `rounded-2xl` (`NewsGallery.tsx:53`). Унификация — открыто (в #77, токены цвета, не входила).
...
- **Строка и заголовок на фото в «Главном».** `NewsCard.tsx:41` (категория · дата, `text-white/75`)
  ...
  тёмного фона правила надзаголовка и заголовка карточки цвет не задают — открытый вопрос
  «Поверхностей» (текст на тёмном фоне).
...
- **Тёмная тема не поддерживается.** В `src/styles.css` объявлены вариант `dark` (`:5`) и блок
  `.dark` с парами токенов (`:146–169`), но класс `.dark` нигде не ставится — пары не применяются;
...
- **`--card-hover` — псевдоним `--news-card-hover`.** `--card-hover: var(--news-card-hover)`
  (`src/styles.css`, #77): роль «наведение карточки» получила своё имя, значение прежнее.
  Переименовать источник — после решения о фоне карточки.
- **Заглушки «нет фото» на токене фона карточки.** `NewsCoverPlaceholder.tsx:17` и
  `PersonPhotoPlaceholder.tsx:12` — `bg-news-card`, тот же токен, что у фона карточки (открыто):
  решение о фоне карточки потянет заглушки.
