...
на **боевой** бакет — локального аналога нет. Следствия:
...
- Guard в `src/routes/admin/_authed/route.tsx` — навигационный, **не граница
  безопасности**. Каждая серверная функция админки обязана проверять сессию
  самостоятельно: эндпоинты `createServerFn` вызываются по HTTP напрямую.
- Секреты (`DATABASE_URL`, ключи S3, `SESSION_SECRET`) в удалённую сессию
  агента не передаются; любая работа, требующая живой БД или S3, выполняется
  только в локальной сессии на машине Антона.
