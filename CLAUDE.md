...
**Исключение (согласовано 09.09.2026, витрина Устава и реквизиты):** `src/routes/federation.tsx` — убран условный рендер `CharterPageAside` (компонент удалён); `src/routes/federation.about.tsx` — добавлен блок «Реквизиты» из `ORG_REQUISITES` (`src/lib/site.ts`); `src/components/site/SiteFooter.tsx` — добавлена строка с полным наименованием и ОГРН; `src/data/mock.ts` — адрес читается из `ORG_REQUISITES`. Новая страница `src/routes/federation_.charter.text.tsx` и `src/components/site/DocumentFileRow.tsx` — зона Claude Code. `src/components/site/FederationSidebar.tsx` — добавлен проп `activeHref` для подсветки пункта на страницах, вложенных по адресу под пункт меню (`/federation/charter/text`); разметка не менялась. `src/routes/federation.tsx` — раскладка переведена на сетку трёх равных колонок (`lg:grid-cols-3`, зазор `gap-6`), основная колонка `col-span-2`; ширина боковой колонки в пикселях убрана. `src/components/site/FeaturedNewsSection.tsx` — обёрткам малых карточек добавлен `md:min-h-0`, чтобы высота строк сетки задавалась большой карточкой, а не пропорцией обложки. Остальная разметка перечисленных файлов — по-прежнему зона Lovable.
